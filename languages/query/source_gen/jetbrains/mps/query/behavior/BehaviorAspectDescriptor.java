package jetbrains.mps.query.behavior;

/*Generated by MPS */

import jetbrains.mps.smodel.runtime.BehaviorDescriptor;
import java.util.Arrays;
import jetbrains.mps.smodel.runtime.interpreted.BehaviorAspectInterpreted;

public class BehaviorAspectDescriptor implements jetbrains.mps.smodel.runtime.BehaviorAspectDescriptor {
  private static String[] stringSwitchCases_846f5o_a0a0a = new String[]{"jetbrains.mps.query.structure.MqlArithmetic", "jetbrains.mps.query.structure.MqlAssignment", "jetbrains.mps.query.structure.MqlBoolLiteral", "jetbrains.mps.query.structure.MqlBoolType", "jetbrains.mps.query.structure.MqlClosure", "jetbrains.mps.query.structure.MqlComma", "jetbrains.mps.query.structure.MqlCondition", "jetbrains.mps.query.structure.MqlDotExpression", "jetbrains.mps.query.structure.MqlFunctionType", "jetbrains.mps.query.structure.MqlIntLiteral", "jetbrains.mps.query.structure.MqlIntType", "jetbrains.mps.query.structure.MqlLinkSelector", "jetbrains.mps.query.structure.MqlListType", "jetbrains.mps.query.structure.MqlNodeType", "jetbrains.mps.query.structure.MqlNullLiteral", "jetbrains.mps.query.structure.MqlNullType", "jetbrains.mps.query.structure.MqlPackage", "jetbrains.mps.query.structure.MqlParameter", "jetbrains.mps.query.structure.MqlParameterReference", "jetbrains.mps.query.structure.MqlParentheses", "jetbrains.mps.query.structure.MqlPropertySelector", "jetbrains.mps.query.structure.MqlQuery", "jetbrains.mps.query.structure.MqlStringLiteral", "jetbrains.mps.query.structure.MqlStringType", "jetbrains.mps.query.structure.MqlThis", "jetbrains.mps.query.structure.MqlTriplex", "jetbrains.mps.query.structure.MqlUnary"};

  public BehaviorAspectDescriptor() {
  }

  public BehaviorDescriptor getDescriptor(String fqName) {
    switch (Arrays.binarySearch(stringSwitchCases_846f5o_a0a0a, fqName)) {
      case 21:
        return new MqlQuery_BehaviorDescriptor();
      case 5:
        return new MqlComma_BehaviorDescriptor();
      case 6:
        return new MqlCondition_BehaviorDescriptor();
      case 19:
        return new MqlParentheses_BehaviorDescriptor();
      case 26:
        return new MqlUnary_BehaviorDescriptor();
      case 0:
        return new MqlArithmetic_BehaviorDescriptor();
      case 9:
        return new MqlIntLiteral_BehaviorDescriptor();
      case 22:
        return new MqlStringLiteral_BehaviorDescriptor();
      case 2:
        return new MqlBoolLiteral_BehaviorDescriptor();
      case 25:
        return new MqlTriplex_BehaviorDescriptor();
      case 14:
        return new MqlNullLiteral_BehaviorDescriptor();
      case 24:
        return new MqlThis_BehaviorDescriptor();
      case 12:
        return new MqlListType_BehaviorDescriptor();
      case 10:
        return new MqlIntType_BehaviorDescriptor();
      case 23:
        return new MqlStringType_BehaviorDescriptor();
      case 3:
        return new MqlBoolType_BehaviorDescriptor();
      case 8:
        return new MqlFunctionType_BehaviorDescriptor();
      case 15:
        return new MqlNullType_BehaviorDescriptor();
      case 20:
        return new MqlPropertySelector_BehaviorDescriptor();
      case 7:
        return new MqlDotExpression_BehaviorDescriptor();
      case 13:
        return new MqlNodeType_BehaviorDescriptor();
      case 16:
        return new MqlPackage_BehaviorDescriptor();
      case 17:
        return new MqlParameter_BehaviorDescriptor();
      case 18:
        return new MqlParameterReference_BehaviorDescriptor();
      case 11:
        return new MqlLinkSelector_BehaviorDescriptor();
      case 4:
        return new MqlClosure_BehaviorDescriptor();
      case 1:
        return new MqlAssignment_BehaviorDescriptor();
      default:
        return BehaviorAspectInterpreted.getInstance().getDescriptor(fqName);
    }
  }
}
